...
class Monitoring_mss(Monitoring):
    def __init__(self, monitor_id=1, crop_size=224, capture_size=224):
        super().__init__()
        self.crop_size = crop_size  # Final output size (always 224 for model)
        self.capture_size = capture_size  # Capture region size (can be larger)
        self.monitor_region = self._get_monitor_region(monitor_id, capture_size)
        self.sct = None
    ...
    @staticmethod
    def _get_monitor_region(monitor_id=1, capture_size=224):
        with mss() as sct:
            monitor = sct.monitors[monitor_id]
            object_size_h_ratio = capture_size / 1080
